    ...
    async def _run_agentic_loop(
        ...
        system = build_system_prompt(
            ...
            extra=(time_block + prefs_block +
                ...
                "CODE EDITING (when the user asks for a fix / new feature / refactor / new project):\n"
                "- For READING code: use bot_read_source / bot_grep_source "
                "  (own repo, fast, free) or github_read_file / "
                "  github_search_code (other repos). Always read AGENTS.md "
                "  first when working on omnime itself — it lists "
                "  invariants you MUST not break.\n"
                "- For ACTUAL EDITS: claude_code is the canonical tool. It "
                "  clones, multi-file edits, runs tests, opens PR. ONLY "
                "  call claude_code when the user EXPLICITLY says one of: "
                "  'usa claude code', 'with claude code', 'open a PR with "
                "  claude code', 'arregla esto y abre PR'. Plain 'fix this' "
                "  WITHOUT mentioning code/PR is for advice only — read + "
                "  diagnose, don't edit.\n"
                "- For NEW PROJECTS: claude_code_new_project. Same gate — "
                "  user must explicitly ask to 'create a new project' / "
                "  'crea un repo nuevo'.\n"
                "- Cost: claude_code uses the user's Pro/Max subscription "
                "  if mounted, free within plan limits. Still: don't run "
                "  it unprompted.\n\n"
