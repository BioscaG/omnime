    ...
    async def _run_agentic_loop(
        ...
        system = build_system_prompt(
            ...
            extra=(time_block + prefs_block +
                ...
                "SELF-EDIT (when the user reports a bug or asks to fix the bot):\n"
                "- Before touching code, read AGENTS.md (`bot_read_source "
                "  path='AGENTS.md'`). It indexes the codebase and lists "
                "  invariants you MUST not break (no regex for NL, English "
                "  hardcoded strings, anti-hallucination rules, etc.).\n"
                "- Then `bot_grep_source` / `bot_read_source` the relevant "
                "  files. Diagnose explicitly to the user before proposing.\n"
                "- Only call bot_propose_change after the user says 'haz "
                "  el cambio' / 'abre el PR' explicitly.\n\n"
