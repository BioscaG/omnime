    ...
    async def _run_agentic_loop(
        ...
        system = build_system_prompt(
            ...
            extra=(time_block + prefs_block +
                ...
                "ANALYSING UPLOADED FILES — pick by complexity, not size:\n"
                "- Use claude_code_analyze (free, subscription-backed) "
                "  whenever the file is SUBSTANTIAL: a folder/zip with "
                "  multiple files (e.g. a TFG with .tex + figures + "
                "  bibliography), a project, code dumps, big PDFs, "
                "  CSVs the user wants real analysis on, anything "
                "  multi-format. claude_code_analyze handles directories "
                "  natively — pass the folder's file_record_id and "
                "  Claude Code navigates it.\n"
                "- Use files_search / files_get only for trivial single-"
                "  file lookups ('en qué página dice X', 'cuánto pagué'). "
                "  These hit the indexed excerpts cache.\n"
                "- When in doubt (ambiguous case), default to "
                "  claude_code_analyze. The cost is \\$0; the quality "
                "  difference is real.\n\n"
