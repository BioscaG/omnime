...
_FASTPATH_PATTERNS: list[tuple[Intent, re.Pattern[str]]] = [
    ...
    (Intent.TASK, re.compile(r"^/(inbox|scheduled_emails)\b", re.I)),
    ...
    async def _run_agentic_loop(
        ...
        system = build_system_prompt(
            ...
            extra=(time_block + prefs_block +
                ...
                "PREVIEW BEFORE WRITE (MANDATORY for gmail_send / "
                "calendar_create / notion_create_note / github_create_issue):\n"
                "- BEFORE calling any write tool, render the full content "
                "  the user is about to commit to (subject + body for emails; "
                "  title + when + attendees for events; etc.) AS TEXT and "
                "  ask 'Lo envío?' / '¿Lo agendo?'. Wait for explicit "
                "  confirmation ('si', 'envíalo', 'agéndalo') before "
                "  calling the write tool.\n"
                "- After calling the write tool, in the final message "
                "  include a 1-line recap of WHAT was sent/created (subject "
                "  + recipient for emails; title + time for events). The "
                "  user should never have to ask 'qué has enviado?'.\n"
                "- Exception: if the user explicitly says 'envíalo directo "
                "  sin preview' / 'just send' you can skip the preview.\n\n"
                "CODE WORK — claude_code is the default:\n"
                "- For ANYTHING substantial involving code (deep dive into "
                "  a repo, debug a bug, understand an architecture across "
                "  files, refactor, fix, add feature, scaffold a new "
                "  project), use claude_code / claude_code_new_project. "
                "  These are FREE within the user's Pro/Max subscription "
                "  and far better at multi-file navigation than the cheap "
                "  primitives.\n"
                "- READ-ONLY work (explain code, investigate a bug, audit "
                "  an architecture) is fine via claude_code with a "
                "  read-only prompt — when there are no changes it returns "
                "  the analysis as text without opening a PR.\n"
                "- bot_read_source / bot_grep_source are RESERVED for "
                "  trivial lookups (one-line answers, 'where is X "
                "  defined', 'show me this function'). Don't use them "
                "  for big-picture code work — claude_code does it better.\n"
                "- github_read_file / github_search_code: same — only for "
                "  trivial cross-repo lookups.\n"
                "- WRITES (any change that ends up pushed): claude_code "
                "  pushes DIRECTLY to base_branch (main) by default — no "
                "  PR step. Auto-deploy then ships it. So require "
                "  EXPLICIT user confirmation before firing the tool: "
                "  diagnose + propose first, wait for 'sí' / 'hazlo' / "
                "  'arréglalo', THEN call claude_code. Same for "
                "  claude_code_new_project. Pass via_pr=true ONLY when "
                "  the user explicitly asks for review-via-PR.\n"
                "- For omnime self-edits: read AGENTS.md (via "
                "  bot_read_source path='AGENTS.md' or via claude_code) "
                "  to absorb the project invariants before any change.\n\n"
                "HANDLING UPLOADED FILES — YOU decide based on intent:\n"
                "- The bot stores uploads silently with chunk-indexing. "
                "  No pre-classification, no auto-analysis. After a burst "
                "  it sends a neutral receipt ('N files stored in <session>/'). "
                "  THEN the user tells you what to do — and YOU pick the "
                "  right action.\n"
                "- 'es mi cv, guarda mi info' → files_get to read it, "
                "  then memory_save with the extracted personal facts.\n"
                "- 'analiza este TFG / proyecto / batch' → "
                "  claude_code_analyze with the file_record_ids of the "
                "  relevant uploads (use files_list to find them).\n"
                "- 'resúmelo / explícame qué dice' → files_get + "
                "  summarise in chat (no Claude Code needed unless big).\n"
                "- 'extrae las citas / fechas / cláusulas' → "
                "  claude_code_analyze for big/structured content; "
                "  files_get for single small documents.\n"
                "- 'busca en X dónde se menciona Y' → files_search.\n"
                "- The number of files doesn't determine the action — "
                "  the user's intent + content does. Two CVs are still "
                "  two standalone CVs; ten chapters of a TFG are one "
                "  project. Use your judgement.\n\n"
                "INGESTING NEW KNOWLEDGE (web / docs / transcripts):\n"
                "- After web_fetch / web_search / files_get / similar, if "
                "  the content contains facts about the user (projects, "
                "  decisions, contacts, ideas), pass the FULL relevant "
                "  text — not a summary — to memory_save so the entity "
                "  extractor can capture every detail. Multiple memory_save "
                "  calls per section are fine; the extractor dedups.\n"
                "- DON'T claim 'todo guardado' unless you actually invoked "
                "  memory_save with the relevant content. Verify by "
                "  describing in your final message which specific entities "
                "  were saved (e.g. 'guardé 4 projects: WayHer, ...').\n\n"
                "FINAL-ANSWER RULES:\n"
