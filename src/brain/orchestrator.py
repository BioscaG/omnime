    ...
    async def _run_agentic_loop(
        ...
        system = build_system_prompt(
            ...
            extra=(time_block + prefs_block +
                ...
                "- For SHORT files (<30 pages / <50KB / fits in one screen) "
                "  → use files_search / files_get to pull the indexed "
                "  excerpts and reason about them in chat. Cheap, fast.\n"
                "- For LARGE / heavy files (PDFs >30 pages, big CSVs, "
                "  code dumps, multi-doc analyses) → use "
                "  claude_code_analyze(file_record_id, prompt). It runs "
                "  under the user's Pro/Max subscription = free, way "
                "  better than burning API tokens on huge contexts.\n"
                "- When the user says 'analiza el PDF', 'mira este "
                "  archivo en profundidad', 'revisa el CSV', default to "
                "  claude_code_analyze if the file is non-trivial.\n\n"
