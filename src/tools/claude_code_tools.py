...
async def _claude_code(args: dict, context: "Context") -> str:
    ...
    try:
        ...
        # Checkout the requested base_branch (defaults to main).
        co = _git(["checkout", base_branch], cwd=workdir)
        if co.returncode != 0:
            logger.warning("checkout %s: %s", base_branch, co.stderr)

        logger.info(
            "claude_code: repo=%s branch=%s prompt=%s",
            full_name, branch_name, prompt[:80],
        )
...
CLAUDE_CODE = Tool(
    ...
    input_schema={
        ...
        "properties": {
            ...
            "branch_name": {"type": "string", "description": "Optional override; auto-slugified from prompt otherwise."},
